...
int main(int argc, char *argv[])
{
  ...
  if (command == "init") {
    ...
    for(int i = 2; i < argc-1 && argv[i][0] == '-'; ++i){
      if(argv[i][1] == 'p') pretty_print = true;
    }
    
    if(argv[argc-1][0] == '-'){
      std::cerr << "Invalid object/type/revision \n";
      return EXIT_FAILURE;
    }
    ...
    if(write_obj && compress(in_f, dest, total_bytes, prefix) != Z_OK){
